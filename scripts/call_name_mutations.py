#!/usr/bin/env python3
"""Mutation controls for ADR-0087: a call names a term.

Each mutant undoes one piece of how a call that names a view, a page, an
event or an effect is refused: the refusal, the binding that shares the name,
and the function or type that does. The tests in `calls_name_terms.rs` must
then fail.

Run from the repository root; `just e10-call-names` records the output. The
...
MUTANTS = [
    ...
    (
        ...
        "        .any(|ns| false && found(ns) != Resolution::Unresolved)\n    {\n        return None;",
    ),
]

TESTS = [
    ["cargo", "test", "--quiet", "--locked", "-p", "pw-core", "--test", "calls_name_terms"],
]
