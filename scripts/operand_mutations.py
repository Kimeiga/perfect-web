#!/usr/bin/env python3
"""Mutation controls for ADR-0043: an operator's operands are typed, and
`Float.from_int`.

A test that passes with the mechanism removed is not evidence for it. Each
mutant undoes one piece of ADR-0043, and at least one test must then fail or
not build.

Run from the repository root; `just e10-operands` records the output. The
sources are restored after every mutant, whatever happens.
"""

import pathlib
import re
import subprocess
import sys

ROOT = pathlib.Path(__file__).resolve().parent.parent
VALUES = ROOT / "compiler/pw-core/src/values.rs"
WASM = ROOT / "compiler/pw-core/src/backend/wasm.rs"

# (what is undone, file, anchor, replacement)
MUTANTS = [
    (
        "a comparison's sides are not related",
        VALUES,
        "            BinOp::Cmp(_) => vec![self.operand(id, rhs, &right, &self.of(lhs))],",
        "            BinOp::Cmp(_) => Vec::new(),",
    ),
    (
        "arithmetic's left side need not be a number",
        VALUES,
        "            _ => vec![\n                self.number(id, lhs, &left),\n",
        "            _ => vec![\n",
    ),
    (
        "arithmetic's right side need not match its left",
        VALUES,
        "                self.number(id, lhs, &left),\n"
        "                self.operand(id, rhs, &right, &self.of(lhs)),\n",
        "                self.number(id, lhs, &left),\n",
    ),
    (
        "a logical operator's left side need not be a Bool",
        VALUES,
        "                    self.operand(id, lhs, &left, &b),\n",
        "",
    ),
    (
        "`!` takes any type",
        VALUES,
        '                    UnOp::Not => ("the operand of `!`", Some(Ty::Primitive(Primitive::Bool))),',
        '                    UnOp::Not => ("the operand of `!`", Some(Ty::Unknown)),',
    ),
    (
        "an `if` condition is not related",
        VALUES,
        "                out.push(self.operand(\n"
        "                    id,\n"
        "                    *cond,\n"
        '                    "the condition of `if`",\n'
        "                    &Ty::Primitive(Primitive::Bool),\n"
        "                ));",
        "                let _ = cond;",
    ),
    (
        "an untyped operand is refused",
        VALUES,
        "            Ty::Unknown | Ty::Parameter { .. } | Ty::Var(_) => {\n"
        "                Outcome::Undecided(Undecided::Unknown)\n"
        "            }",
        "            Ty::Unknown | Ty::Parameter { .. } | Ty::Var(_) => Outcome::Disagree {\n"
        '                expected: "Int or Float".to_string(),\n'
        '                actual: "unknown".to_string(),\n'
        "            },",
    ),
    (
        "`Float.from_int` reads its Int as unsigned",
        WASM,
        "I::F64ConvertI64S,",
        "I::F64ConvertI64U,",
    ),
]

TESTS = [
    ["cargo", "test", "--quiet", "--locked", "-p", "pw-core", "--test", "value_relations"],
    ["cargo", "test", "--quiet", "--locked", "-p", "pw-conformance", "--test", "stdlib"],
    ["cargo", "test", "--quiet", "--locked", "-p", "pw-conformance", "--test", "computation"],
]


def run_tests():
    """(built, passed, failed) over every test command."""
    built, passed, failed = True, 0, 0
    for cmd in TESTS:
        r = subprocess.run(cmd, cwd=ROOT, capture_output=True, text=True)
        out = r.stdout + r.stderr
        m = re.search(r"test result: \w+\. (\d+) passed; (\d+) failed", out)
        if m is None:
            built = False
            continue
        passed += int(m.group(1))
        failed += int(m.group(2))
    return built, passed, failed


def main():
    built, passed, failed = run_tests()
    print(f"baseline: {passed} passed, {failed} failed")
    if not built or failed or not passed:
        print("FAIL: the unmutated baseline is not green; no mutant can mean anything")
        return 1

    survivors = 0
    for what, path, anchor, replacement in MUTANTS:
        original = path.read_text()
        if original.count(anchor) != 1:
            print(f"{what}: ANCHOR NOT FOUND EXACTLY ONCE in {path.name}")
            survivors += 1
            continue
        try:
            path.write_text(original.replace(anchor, replacement, 1))
            built, passed, failed = run_tests()
        finally:
            path.write_text(original)
        if not built:
            verdict = "KILLED (does not build)"
        elif failed:
            verdict = f"KILLED ({failed} of {passed + failed} tests fail)"
        else:
            verdict = "SURVIVED"
            survivors += 1
        print(f"{what}: {verdict}")

    print(f"{len(MUTANTS) - survivors} of {len(MUTANTS)} mutants killed")
    return 1 if survivors else 0


if __name__ == "__main__":
    sys.exit(main())
