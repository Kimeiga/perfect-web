...
fn encode(component_id: &str, function: &Function, program: &Program) -> Result<String, String> {
    ...
    e.function(function)
        .map_err(|why| format!("`{component_id}` is not a JavaScript module: {why}"))?;
    Ok(e.finish(component_id, function))
}
...
impl Emitter<'_> {
    fn finish(self, component_id: &str, function: &Function) -> String {
    ...
    fn instr(&mut self, i: &Instr) -> Result<(), String> {
        ...
        match i {
            ...
            Instr::Call { .. } => {
                return Err("a call the lowering did not inline".into());
            }
