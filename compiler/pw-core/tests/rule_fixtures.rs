...
fn every_rule_fixture_reports_exactly_what_it_declares() {
    ...
    let files: Vec<(String, String)> = all.iter().map(|(n, s, _)| (n.clone(), s.clone())).collect();
    let results: std::collections::HashMap<_, _> = check_sources(&files).into_iter().collect();

    let mut wrong = Vec::new();
    for (name, _, expect) in &all {
...
fn at_least_one_fixture_per_directory_is_clean() {
    ...
    let all = fixtures();
    let clean = all.iter().filter(|(_, _, e)| e == "clean").count();
    assert!(
        clean >= 1,
        "no valid case — the suite cannot distinguish a rule from a ban"
    );
}
...
fn the_unsafe_audit_rule_checks_both_halves_and_the_target() {
    ...
    // is not.
    let (n, src, _) = by_name("unresolved-attribution.pw");
