...
| [0024](DECISIONS/ADR-0024-optimistic-and-rollback-bind-with-a-lambda.md) | `optimistic` / `rollback` bind their subject with a **lambda** — `optimistic cart => cart.add(..)` — rather than a new binder form | Proposed | E10 | the architect locked the semantic requirement (no magic binding) and left the surface syntax open; a lambda is the binder Pleris already has |
