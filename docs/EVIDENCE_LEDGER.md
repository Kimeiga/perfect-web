...
| P0 | "Nine of the twenty-nine invariants the corpus exercises are enforced against programs the fixtures did not anticipate — different control flow, different depth, different construct — and the figure is measured rather than claimed." | E9, E9C, E5 | `examples/generality/**`; `just generality` | rust 1.97.1 | **measured** | "the analyses generalize" — nineteen are **untested** and one is known narrow. This number, not 44/44, is the one that bounds any claim about the analyses |
