...
| P0 | "Every invariant the corpus exercises has been challenge-tested: a violation written in a shape its fixture does not have is caught, and a valid near-neighbour differing only in the thing the rule is about still compiles." | E9, E9C, E5, E6, E7 | `examples/generality/**`; `just generality` | rust 1.97.1 | **measured** | "the analyses are general" — 29/29 means each survived a program its fixture did not anticipate, which is the strongest thing a finite suite can say and is not generality |
| P0 | "The eight invariants a headline claim rests on have a full challenge matrix — direct, indirect, valid neighbour, and semantic-preserving transformations — with the relevant and irrelevant dimensions both written down." | E5, E9, E9C | `examples/generality/*/DIMENSIONS.md` | rust 1.97.1 | **measured** | that the dimensions are exhaustive — each matrix names what it does **not** cover |
| P0 | "Every rejected fixture emits its declared invariant and no unexplained extra semantic diagnostic." | E2–E9C | `every_rejected_fixture_emits_only_its_own_defect` | rust 1.97.1 | **measured** | that this was always true — two false positives were sitting in fixture output, satisfying every existing assertion, until it was measured |
| P0 | "No program in any suite panics the compiler, across nine generator families, and an unexpected panic is reported as a marked internal error with a reproduction line rather than a silent exit." | E9C | `compiler/pw-core/tests/robustness.rs`; `just robustness` | rust 1.97.1 | **measured** | "the compiler cannot crash" — no coverage-guided fuzzer has run, and reverting the arity fix left the broad generators green while only the targeted one went red |
| P0 | "Resume artifacts are accepted only under exact compatible identities or explicit checked migrations; incompatible mixed-version artifacts fail closed through tested recovery paths." | E7V | `runtime/pw-resume`; `docs/evidence/E7V/deployment-matrix.txt` (`just resume-matrix`) | rust 1.97.1 | **measured** | "resumption is safe across deployments" — the decision is **not wired into a running browser**. It is a decision function with a tested contract, like `pw-tasks` and `pw-resource` (ADR-0016), and Marko still implements resumption itself (ADR-0002) |
| P0 | "A capture whose type the build cannot name is a compile error, so a manifest is never written from a guessed schema." | E7V | `PW5016`; `examples/generality/capture_schema_disagrees/` | rust 1.97.1 | **measured** | that runtime version skew is a compile error — it is not, and cannot be; that half is a typed runtime decision |
| P0 | "Known gaps are executable: a program that violates an invariant and is not caught is checked in as a file that must compile clean, so `just ci` fails the moment the gap closes." | E9C | `examples/generality/**/slips-through.pw` | rust 1.97.1 | **measured** | that the gap list is complete — it holds the gaps that have been *found*, and the phase that created it found three defects nobody had reasoned their way to |
| P0 | "The pre-change text of nine of the ten modified fixtures is still rejected, for the same invariant, without the declaration that was added — so the repairs removed the obstruction and not the defect." | E2B–E6 | `docs/CORPUS.md`; `compiler/pw-core/tests/corpus_history.rs` | rust 1.97.1 | **measured** | that the corpus is unchanged — ten fixtures changed and each has a row |
| P0 | "Every diagnostic's semantic identity is read from the registry, and each fixture declares the invariant symbol it expects, so a number cannot acquire a second meaning." | E5 | `compiler/pw-core/src/codes.rs`; `a_fixtures_declared_symbol_matches_the_symbol_it_is_caught_by` | rust 1.97.1 | **measured** | that this was always true — `PW0323` meant three things at once and every code-level assertion passed the whole time |
