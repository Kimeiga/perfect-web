...
Twenty-five measurements in this project produced plausible, favourable results
...
| E7-R runtime | the store page's Add buttons were attached, and one of them worked | a template-scoped `ElementId` names a position in the TEMPLATE, and a position inside `{#each}` has one document instance per item — all three Add buttons carry `data-pw="0"`. The runtime kept a `Map` from id to element, so it silently held the LAST one: the third button worked and the first two were inert. Clicking the first did nothing, which reads as a page still loading. Found by a test that clicks `.first()` |
