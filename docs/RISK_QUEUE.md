...
| E9 types | E9-V2 — *generic calls instantiate and unify through resolved types* — is a gate a program can be written against | **no callable can be generic.** `fn id<T>(x: T) -> T` does not parse: `PW0007`, *expected a declaration, found `<`*. The grammar calls `type_params()` for `opaque type` and for `effect`, and for nothing else — so `fn`, `query`, `command`, `subscription`, `resource` and `task` have no type parameters at all. The architect's V2 discriminator is `identity<T>(T); identity(Store) → infer T = Store`, and **it cannot be written**. V2 has no surface syntax to be tested against, so closing it needs either callable generics added to the language or the gate restated in terms of the generics that do exist (`Secret<C>`, `Session<S>`, `List<T>`). That is a language-surface decision, not a repair. Found while wiring `Signature`'s resolved fields, which thread a binder and the declaration's `type_params` so a bound `T` resolves to its binder rather than being reported unresolved. That plumbing is correct and **exercises nothing**, because `decl.type_params` is empty for every callable — which is itself the shape worth noting: correct machinery for a case the language cannot express looks identical to correct machinery that runs. Pinned by `a_callable_cannot_declare_a_type_parameter`, whose discriminator parses `opaque type Secret<C>` so the assertion is not merely a parser that rejects `<` |
| E9 types *(fixed)* | a type declared in one file is visible to a match in another, because one invocation is one program | **it was visible because the type environment was keyed by bare NAME, so a module saw types it never imported.** `the_environment_spans_every_file_checked_together` declared `type T` in `module a` and matched on a `T` in `module b` with **no import**, and the match was checkable — the same defect as calling a name you never imported (`PW0021`), one layer over, holding up the test that made assumption A-009 testable. Two modules declaring one type name collapsed the same way: `Env::adts` and `Env::ctor_names` kept the last, so a match over `alpha.Status` was checked against `beta.Status`'s constructors with no diagnostic, because both spellings agreed. Repaired 2026-08-21 on the architect's ruling that **`TypeEnv` never discovers identity; it only supplies declaration facts for an identity it is handed** — the maps are keyed by `DefId`, there is no by-name entry point, and a scrutinee's declared type is resolved through `ResolvedType` before the environment is consulted. `locals` carries the `DeclaredType` rather than a re-rendered string, because re-parsing a rendered type is where arguments get dropped. The control the ruling asked for is frozen: two modules declaring `Status` with overlapping constructors, where **reordering the modules must not change the verdict** — which is what proves `by_adt_name` is dead rather than wrapped behind a new API. The fixture's own control moved in the right direction too: checking `b` alone now reports `PW0020` for the missing module instead of silence, so *the type is unknown* is stated rather than inferred from an empty list |
| E9 types | E9 is COMPLETE — *permanent value type checker* — and charter §14 M9A lists `opaque nominal types` and `unification-based inference` among its contents | **no call site is type-checked.** Not the argument types, not the arity, not the result: `takes_str(42)` for `fn takes_str(s: String)`, `fn wrong_return() -> String { 42 }`, `wrong_arity(1)` for a two-parameter function, and — the case with no alternative explanation — `takes_store(makes_cart())` where `Store` and `Cart` are two `type` declarations in ONE module, both resolvable, neither ambiguous. `pw check` reports nothing for any of them. The type machinery is real and is applied to specific relations, each with its own controls: `{#each}` capture element types, handler signatures (`PW0602`), an optimistic transition's target and value (`PW0331`), match exhaustiveness, effect rows, privacy labels. So this is not *types are unimplemented* — it is that **ordinary application of a function is not among the relations checked**, which is the one a reader would assume first from the milestone's name. Found while executing the architect's ruling of 2026-08-20 to *make opaque/nominal compatibility use resolved `DefId`, not representation* — a step that presupposes a compatibility check to repair. The narrower finding it came from (two `opaque type SessionId = String` declarations interoperating) was a symptom and would have been the wrong repair: an opaque-identity rule would have produced the right verdict on that one case from a mechanism unrelated to the actual gap, which is the failure this project keeps deleting. **Claim versus witness at a gate rather than in a fixture.** The evidence-reachability audit asked which observer could witness each fixture's claim; nothing asked it of a milestone headline. E9's evidence is real and witnesses what it covers — `each-typing`, `koka-parity`, the effect oracle — and none of it witnesses call-site typing. Pinned by `a_call_site_is_not_type_checked`, which fails when the check exists. **First repair, same day: `PW0604`, call arity** — the piece that needs no inference, so it could be correct against the whole corpus immediately rather than waiting for a type comparison. Argument TYPES are deliberately not in it: a rule that fires on some mismatches reads as a rule that catches them. Two things it got wrong on its first runs, both caught by its own controls rather than by review. It enumerated callable `DeclKind`s and needed the callee's `Decl` to read `kind` — looked up in the CURRENT unit only, so every cross-module call fell through and the rule silently did not apply to them; repaired by asking a different question entirely (a non-callable has no `Signature`, so the exclusion is structural and there is no list to fall out of date). And it counted only a call's own arguments, so every `items |> List.map(f)` read as one short — five sites across A-001 and A-016, all correct code, found by the corpus control on the rule's first run. **And on its first run against the REJECTED corpus it found a real defect:** `R-011` calls `Carts.clear(tx, current_session())` against a `clear` declaring one parameter — the fixture describing a library that does not exist, which is the `Stores.find` shape `tests/platform_contracts.rs` was written for. R-011's own header already records the same lesson from 2026-08-10, when `PW0021` learned to examine bare calls and found it importing nothing for a name it used: *a rejected fixture must isolate ONE defect; a second, silent one made it evidence for less than it claimed.* **Two silent second defects in one fixture, found by two different checks three days apart** — that is a fact about the corpus rather than about the file, and it is the strongest argument yet that a fixture's claim needs a witness per claim rather than per fixture |
| E10 methodology *(fixed)* | a text-scanning check keeps checking when the text it scans is regenerated | **the fifth and sixth instances of the vacuity pattern, both caused by one commit of my own.** Moving the emitted types into a `pw:types` package changed every `use types.{..}` line to `use pw:types/types.{..}`. `every_type_an_exported_signature_names_is_declared_in_the_package` matched with `strip_prefix("use types.{")`, so it matched nothing, `missing` stayed empty, and it passed while checking one of its two halves. In the same change `just e8-wit` counted types with `grep -cE '^    (record|variant|type) '` — a fixed four-space indent — and reported **0 types on the ABI** for a package declaring seventeen. Neither failed. One reported a pass and the other reported a number, and a number is the more dangerous of the two because it looks like a measurement. Repaired by matching shape rather than spelling (`use <anything ending in types>.{`), by an indent-insensitive count, and — the part that generalizes — by a **non-vacuity floor** on the loop, so a scan that examines nothing fails instead of passing. Same countermeasure as the `checked >= 3` floor added to the world-import check hours earlier, which is the tell: the pattern recurs faster than instances of it get fixed |
