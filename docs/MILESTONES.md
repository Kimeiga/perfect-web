...
| **E8** | Rust capability host, WIT worlds, Wasmtime execution | in progress — `runtime/pw-host` decides admission on data (topology, placement, audit) with 15 controls, and `just e8-host` runs the artifact audit against real components: the ordinary `std` guest is refused for 14 `wasi:*` interfaces its WIT world never declared. Remaining: instantiation through the granted handles, and the WIT world generation |
