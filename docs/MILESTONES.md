...
| **E9** | Permanent value type checker and algebraic effect compiler (9A–9D) | **REOPENED 2026-08-21** — evidence gap discovered during E10. Everything previously evidenced remains evidenced and is not rewritten: corpus 24/24 accepted and 46/46 rejected; **E9-K** effect-oracle conformance with the four shared properties asserted beside Koka's OUTCOME 1 and the four recorded divergences still tested as divergences; no crate depends on Koka; interactive checking **37 ms** for the 36-file store program, cold and after an edit and while producing a diagnostic; 3,600 fuzz executions, 0 findings; `{#each}` capture typing. **What was never evidenced:** ordinary call sites are not type-checked. `takes_str(42)`, `fn wrong_return() -> String { 42 }` and `takes_store(makes_cart())` all pass `pw check` — the last with two `type` declarations in one module, both resolvable, neither ambiguous. Architect ruling, 2026-08-21: *a milestone called* permanent value type checker *cannot honestly remain complete while all of those pass; that is not an edge case, it is a missing central relation.* The milestone claim was broader than its evidence, which is the evidence-reachability question asked of a gate rather than of a fixture. Closing gates **E9-V1**–**E9-V6** are added rather than history being rewritten; `PW0604` (call arity) is the first and is done. See `docs/NEXT.md` |
| **E10** | Own backends and automatic memory strategy | not started — **carries E10-I**: compile `add_to_cart` through the Pleris→component backend and run it through the E8 host, with no alternate Rust closure path. Deferred from E8's gate, and the gate cannot close without it |
