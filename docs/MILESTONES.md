...
| **E8** | Rust capability host, WIT worlds, Wasmtime execution | **COMPLETE** — gate amended 2026-08-08 by ruling, and the amendment is the honest part. E8's claim is *given a component contract and a Wasm component artifact, the host can determine what authority it may receive, verify what the final artifact actually imports, link only granted interfaces, constrain its resources, and execute or refuse it accordingly* — which does not require the component to have come from Pleris. Eleven controls: topology and placement admission; required capabilities; component-vs-host import distinction; final-artifact audit (the ordinary `std` guest refused for 14 `wasi:*` interfaces it never declared); generated WIT resolving under `wit-parser`; linking only from `Granted`; forbidden imports unlinked; **a permitted guest calling the host and receiving its answer**; a denied one unable to; fuel, memory and table limits; and no `worlds_for` fallback anywhere. The end-to-end *Pleris-compiled* command moved to **E10-I** (`docs/EVIDENCE_LEDGER.md`), because E8 cannot require an artifact only E10 knows how to make |
| **E9** | Permanent value type checker and algebraic effect compiler (9A–9D) | **COMPLETE — re-closed 2026-09-24** after the 2026-08-21 reopening. The closing gates **E9-V1..V6** are met by checks that refuse programs, not by representation. Arity and argument types cover every typed call: path, member, piped, query, construction and policy-term. Generic callables are instantiated per call, with function types and bidirectional lambdas. Nominal identity is by DefId, results are checked through branches, `return` and `?`, unresolved or wrong-arity written types are reported and never compared, and ABI-equal types stay distinct. Evidence: 38 gate tests, 10/10 mutation controls killed, every store-program relation decided (`docs/evidence/E9/value-relations-2026-09-24.md`, ADR-0031). The first run found defects in the milestone demo, the accepted corpus and three libraries; corpus version C8 records the repairs. Everything evidenced before the reopening stands: corpus 24/24 and 46/46, E9-K oracle conformance, 37 ms interactive checking, 3,600 fuzz executions. **Not claimed:** member existence, sum-type variant constructors, named-argument calls |
| **E10** | Own backends and automatic memory strategy | **gate items 1–5 evidenced 2026-09-25; closing awaits review.** **Gate item 2 met 2026-09-25:** all seven compiled declarations agree with independent Rust references over 300 generated cases each (`docs/evidence/E10/oracle-2026-09-25.md`). The kiokun slice (ADR-0037) runs entry lookup and search over one real shard of kiokun.com. E10-I closed 2026-09-24. `add_to_cart` and `clear_cart` compile to Wasm components and run through the E8 host, and the dev server's Rust closures are deleted (ADR-0032, `docs/evidence/E10/`). All five of the store's commands and queries compile to audited components. **Resumable handler bodies compile to ES modules since 2026-09-25** (ADR-0033): each element carries the capture paths its handler reads, and the browser's arguments are typed by the command component's own parameters. **Gate item 1 met 2026-09-25** (ADR-0034): `pw build` builds the store from source to its template IR, handler modules and five audited components, with neither Koka nor Node on the PATH. **Gate item 3 met 2026-09-25** (ADR-0035): sustained load leaves the server bounded, after three leaks were measured and fixed, and 20,000 compiled calls leave the host's memory flat. **Gate item 5 and task 10 met 2026-09-25:** only effect-row producers make a value affine, the store's 20 value positions carry no ownership, and carried captures serialize deterministically. **Gate item 4 met 2026-09-25:** components 3–4 KB against 5.3 KB for a hand-written Rust `no_std` guest; 21.1 µs per host call against 22.1 µs for the Rust guest; browser activation unchanged against the pre-E10 runtime (21 interleaved samples each, twice). Remaining for the E10 gate:<br>• records, variants and constants written into the region<br>• calls between compiled declarations<br>• branches<br>• replacing `store:data/carts` with compiled Pleris (step 10)<br>• an automatic memory strategy beyond the provisional invocation region |
