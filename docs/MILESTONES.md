...
| **E2B** | Program graph and name resolution *(inserted by architect ruling, 2026-08-06)* | **partial** — module graph, namespaces and import resolution complete; use resolution checked but not yet *integrated* (analyses still compare strings); fixture isolation in progress |
| **E2C** | Core library and platform contracts — signature manifests *(inserted)* | **first slice landed** — 10 modules with real effect rows; the corpus resolves against them. Open: the compiled manifest, and deleting the stand-in tables |
| **E2D** | Source effect inference *(inserted; pulled forward out of E9)* | **first slice landed** — direct, helper-hidden and callback-hidden propagation with the chain in the diagnostic; two codes (undeclared vs forbidden-in-context). Open: geometry read as property access, which needs E2B's member lookup |
...
| **E5** | Privacy-flow, cache-safety, and placement checker | **3 of 5 gate items pass** — set-of-restrictions label algebra with exhaustively verified lattice laws, a placement solver over build/browser/edge/origin, label propagation through declarations, and markup rules. **19 of 44** rejected corpus programs are compile errors. Open: the ≥50 compile-fail count and the store page |
