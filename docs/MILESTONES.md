...
| **E9** | Permanent value type checker and algebraic effect compiler (9A–9D) | **COMPLETE** — gate amended 2026-08-08 by ruling (items 2 and 4 specified a technique where the gate wanted an outcome), then met in full. Corpus 24/24 accepted and 46/46 rejected. **E9-K** effect-oracle conformance: the four shared effect properties asserted on Pleris programs beside Koka's own OUTCOME 1, and the four recorded divergences still tested as divergences. No crate depends on Koka. Interactive checking is **37 ms** for the 36-file store program, cold and after an edit and while producing a diagnostic — so incremental queries have no measurable job yet and were not built. 3,600 fuzz executions, 0 findings. **Largely completed by work pulled forward into E2B/E2C/E2D/E8** rather than by a rewrite, which is allowed: a milestone is a gate, not a schedule |
