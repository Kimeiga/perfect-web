...
| **E10** | Own backends and automatic memory strategy | **in progress — E10-I closed 2026-09-24.** `add_to_cart` and `clear_cart` compile to Wasm components and run through the E8 host, and the dev server's Rust closures are deleted (ADR-0032, `docs/evidence/E10/`). All five of the store's commands and queries compile to audited components. Remaining for the E10 gate:<br>• compiled resumable handler bodies<br>• records, variants and constants written into the region<br>• calls between compiled declarations<br>• branches<br>• replacing `store:data/carts` with compiled Pleris (step 10)<br>• an automatic memory strategy beyond the provisional invocation region |
