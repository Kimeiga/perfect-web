...
| **E2B** | Program graph and name resolution *(inserted by architect ruling, 2026-08-06)* | not started — blocker cleared: `examples/domain.pw` written, so the corpus is a real program. Gate in `docs/milestones/E2B.md` |
| **E2C** | Core library and platform contracts — signature manifests *(inserted)* | not started — deletes the stand-in tables in `check.rs` and `placement.rs` |
