...
| R18 | **Fine-grained updates are mistaken for layout safety** (charter v2 §20) | **MEASURED, REAL** | high | high | A renderer that minimizes DOM writes does nothing about application code that interleaves geometry reads with layout-invalidating writes. Measured at **264x-848x** on identical work. Mitigation per charter: separate measurement and mutation into distinct effects/phases; deny direct synchronous geometry APIs in app packages; forced-layout instrumentation built in Milestone 0 (`spikes/layout-phase-scheduler`); layout regression gates in the store demo (M4); containment and virtualization only where semantically safe; retain an audited escape hatch. **Fallback taken already:** the phase scheduler works as a runtime-enforced API without compile-time proof. |
...
| R19 | **A layout benchmark can silently measure nothing.** Writing an identical value does not invalidate layout, so run 2 onward showed 0.3 ms instead of 79.7 ms — indistinguishable from a successful fix. | `spikes/layout-phase-scheduler` F-2 | Any layout regression gate must salt its writes and assert that the baseline case actually thrashes. A gate that cannot fail is not a gate. |
| R20 | **The charter's preferred forced-layout signal does not exist in current Chrome.** `forcedStyleAndLayoutDuration` is `undefined` on `long-animation-frame` entries in Chrome 150. | `spikes/layout-phase-scheduler` F-3 | Use long-frame count + `blockingDuration` as the proxy. The harness probes for the field and will pick it up automatically if a later Chrome exposes it. |
| R21 | **CSS containment can lose performance when applied where it does not belong.** A contained subtree was marginally *slower* for a forced layout following an unrelated host mutation, while being 4.18x faster for initial subtree layout. | `spikes/layout-phase-scheduler` F-4 | Charter §7.5A's "only when subtree independence is semantically valid" is a performance constraint as well as a correctness one. The compiler must not emit `contain` speculatively. |
